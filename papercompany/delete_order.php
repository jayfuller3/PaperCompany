...
try {
    ...
    $stmt = $conn->prepare("SELECT orderID, orderCust, orderDate, orderNotes, orderTotals FROM orders WHERE orderID=:oidParam");
    $stmt->bindParam(':oidParam', $_GET["oid"]);
    $stmt->execute();
...
<form method="POST" action="store_deleted_order.php">
  Customer ID:<br>
